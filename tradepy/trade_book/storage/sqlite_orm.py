...
class TypeAnnotation:
    ...
    def infer_primitive_type(self):
        ...
        if self.is_union():
            ...
            raise TypeError(
                f"Cannot decide an unique primitive type for {self.anont}"
            )

        if self.is_not_required():
            return TypeAnnotation(
                get_args(self.anont)[0]
...
class Table(Generic[RowDataType]):
    ...
    def __deserialize(self, row: list[Any]) -> RowDataType:
        return cast(
            RowDataType, {f.name: row[i] for i, f in enumerate(self.fields)}
        )
...
values ({", ".join(map(self.__serialize, values))})
    ...
    def insert(
        self, conn: sqlite3.Connection, row: RowDataType
    ) -> sqlite3.Cursor:
...
where {
    ...
    def build_update_sql(
        self, where: dict[str, Any], update: dict[str, Any]
    ) -> str:
        set_clause = ", ".join(
